...
static RECORD_ALL_CLIENTS: Mutex<c_ulong> = Mutex::new(xrecord::XRecordAllClients);
static GLOBAL_CALLBACK: Mutex<Option<Box<dyn FnMut(Event) + Send + Sync>>> = Mutex::new(None);

pub fn listen<T>(callback: T) -> Result<(), ListenError>
where
    T: FnMut(Event) + Send + Sync + 'static,
{
    let keyboard = Keyboard::new().ok_or(ListenError::KeyboardError)?;

    unsafe {
        *KEYBOARD.lock().unwrap() = SendWrapper::new(Some(keyboard));
...
unsafe extern "C" fn record_callback(
    ...
    if let Some(event) = convert(&mut KEYBOARD.lock().unwrap(), code, type_, x, y) {
        let thing = &mut *GLOBAL_CALLBACK.lock().unwrap();
        if let Some(callback) = thing {
            callback(event);
        }
    }
